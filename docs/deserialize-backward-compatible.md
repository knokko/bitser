...
Start by deserializing an instance of `LegacyClasses`, with holds
...
As mentioned in the previous step, the structs, collections, arrays, and
maps will be mostly uninitialized. Since all values and reference targets
should be available now, it's time to populate the empty structs, etc.
